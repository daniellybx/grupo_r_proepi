...
serie <- serie %>%
  mutate(
    media_movel = rollmean(casos, k = 3, fill = NA, align = "right")  # alinhamento à direita
  )

# Gráfico com linha cinza para os casos e linha vermelha para a média móvel
serie %>%
  ggplot(aes(x = semana)) +
  geom_line(aes(y = casos), color = "gray") +
  geom_line(aes(y = media_movel), color = "red", size = 1) +
  labs(title = "Casos semanais e média móvel (3 semanas)",
